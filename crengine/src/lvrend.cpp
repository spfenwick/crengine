...
// general rounded-corner path covers -- solid, inset, outset, dashed,
// dotted, double, groove, or ridge (any mix of these eight is fine, since
// that path handles dashed/dotted sides individually via their own
// perimeter-arc walk regardless of what the other sides are). Used by the
// background-fill code (which has no border-drawing context of its own) to
// decide whether its fill must be rounded to match.
static bool styleHasOnlyRoundableBorders(const css_style_rec_t * style) {
    for (int i=0; i<4; i++) {
        ...
        bool has = bs >= css_border_solid;
        css_length_t bw = style->border_width[i];
        has = has && !(bw.value == 0 && bw.type > css_val_unspecified);
        lUInt32 color = style->border_color[i].type != css_val_unspecified ? style->border_color[i].value : style->color.value;
        has = has && !IS_COLOR_FULLY_TRANSPARENT(color);
        if (has && bs != css_border_solid && bs != css_border_inset && bs != css_border_outset &&
...
// curve so the seam stays correct even where a large radius has already
// pulled the curve in past that flat corner.
//
...
// actually gets painted. That's what lets a call painting only the left
// side (draw_left true) still miter correctly against a top side rendered
// entirely by someone else (draw_top false; e.g. a dashed top via its own
// arc walk), with no gap or double-paint at the seam.
...
// ---------------------------------------------------------------------------
// FreeType-based stroking for round DASHED borders only (round DOTTED borders
// keep using the AABB-quad walk in walkAndDashSegments/fillCircle below).
//
// Phasing a dash run by row/column per side, or filling the axis-aligned
// bounding box of each ~1px step of a rotated quad, is correct (and cheap) on
// the straight sides, where the quad is already axis-aligned, but on corner
// arcs -- or any dash straddling an edge/arc seam -- the bounding box is
// strictly bigger than the true rotated quad, which produces visibly
// rectangular, non-perpendicular dash ends on curves ("stamped with a
// rectangular pen" instead of cut perpendicular to the path).
//
// FT_Stroker is FreeType's own path-to-outline stroker (also used to stroke
// glyph outlines): given a path -- built here from the same edge/arc geometry
// as the AABB walk, with corner arcs expressed as cubic Beziers -- it produces
// a correctly capped/joined stroke outline, which FreeType's own scanline
// rasterizer then turns into an antialiased 8bpp coverage mask. That mask is
// composited via LVDrawBuf::Draw(x,y,bitmap,w,h,palette), the same call used
// for every glyph bitmap in the document, so it picks up the same per-bit-
// depth dithering as text for free, instead of reimplementing it here.
//
// All dash runs for one border (ring or per-side chain) are accumulated into
// a single FT_Stroker pass and rasterized/blitted once, rather than once per
// dash.
// ---------------------------------------------------------------------------
...
// Maps a local distance within one arc segment back to its parametric angle
// t, via the segment's cumulative arc-length table (binary search + linear
// interp between the two bracketing samples) -- the same lookup
// walkAndDashSegments' pointAt() does inline for the AABB-walk case.
static double crArcLocalDistanceToT(const CRPerimeterSeg &sg, double local) {
    if (local < 0) local = 0;
...
static void crPointOnSeg(const CRPerimeterSeg &sg, double local, double &px, double &py) {
    ...
    double t = crArcLocalDistanceToT(sg, local);
    px = sg.cx + sg.crx*cos(t); py = sg.cy + sg.cry*sin(t);
}

// Converts a device-space point into an FT 26.6 fixed-point point relative to
// a bitmap whose top-left device pixel is (originX, originY). FreeType's
// outline rasterizer uses a bottom-up (y-increases-upward) convention --
// empirically verified against this FreeType build, since it's undocumented
// behavior worth pinning down rather than assuming -- so Y is flipped against
// the bitmap height here; this is the only place that flip happens, every
// other coordinate in this file stays in normal top-down device space.
...
// one cubic Bezier -- accurate to a small fraction of a pixel for sweeps up
// to 90 degrees, which covers any single corner here -- via the standard
// control-point-from-tangent construction (control point = endpoint + (4/3
// tan(sweep/4)) * tangent).
static void crEmitSegRange(FT_Stroker stroker, const CRPerimeterSeg &sg, double local0, double local1,
    ...
    double t0 = crArcLocalDistanceToT(sg, local0);
    double t1 = crArcLocalDistanceToT(sg, local1);
...
// path tangent at the run's ends -- even when the run straddles an edge/arc
// seam, since one subpath can chain LineTo/CubicTo across several underlying
// segments, with only a join (set to ROUND on the stroker below) at each
// internal seam.
//
// `closed` is true only for a run that covers the entire perimeter of a
// closed ring (a dash pattern with no visible gap): its start and end points
// are the same physical location, but computed independently -- the start
// via crPointOnSeg, the end via crEmitSegRange's own arc/line formulas -- so
// they can differ by a fraction of an FT unit. Opened as an *open* subpath,
// FT_Stroker would cap both of those near-coincident points with butt caps,
// leaving a visible seam at the join. Opened as *closed* instead, FT_Stroker
// (see FT_Stroker_EndSubPath) compares the last emitted point against the
// subpath's start and, if they don't land exactly on top of each other,
// silently adds the short closing LineTo itself, then joins in/out tangents
// with the same ROUND join used at every other internal seam -- no cap, no
...
static void crEmitDashRun(FT_Stroker stroker, const CRPerimeterSeg *segs, const double *prefix, int numSegs,
                           double d0, double d1, double originX, double originY, double bmpH, bool closed) {
    int last = numSegs - 1;
    ...
    FT_Stroker_BeginSubPath(stroker, &start, closed ? 0 : 1);
...
// the resulting antialiased coverage mask. The canvas is sized from segs[]
// itself (rather than from a caller-supplied box) so this works the same for
// the closed-ring uniform case and the open-chain per-side (mixed-style)
// case: a conservative bound is each edge's two endpoints, and each arc's
// full ellipse bounding box (cx +- crx, cy +- cry) -- looser than the arc's
// actual t0..t1 span, but cheap and always safe.
//
// `fullLoopRun` marks a single run (there can only be one, since a run [0,
// totalLen] leaves no room for any other) that goes all the way around a
// closed ring with no gap -- see crEmitDashRun for why that one run needs to
// be closed rather than opened with butt caps.
                              ...
                              bool fullLoopRun)
{
    ...
    for (int i = 0; i < segCount; i++) {
        ...
        if (!sg.isArc) {
            ...
            minx = std::min(minx, std::min(sg.ex0, ex1)); maxx = std::max(maxx, std::max(sg.ex0, ex1));
            miny = std::min(miny, std::min(sg.ey0, ey1)); maxy = std::max(maxy, std::max(sg.ey0, ey1));
        } else {
            minx = std::min(minx, sg.cx - sg.crx); maxx = std::max(maxx, sg.cx + sg.crx);
            miny = std::min(miny, sg.cy - sg.cry); maxy = std::max(maxy, sg.cy + sg.cry);
        }
    }
    ...
    // _hidePartialGlyphs), which is correct for avoiding ugly half-cut
    // letters at a page break but would make a border vanish outright if a
    // bordered box's edge happens to land past the fold; the old
    // FillRect-based border code never had that failure mode.
...
// Walk a sequence of perimeter segments -- either a closed loop (wrap=true:
// the uniform-ring fast path, all 4 edges + 4 arcs) or an open chain (wrap=
// false: a single side's own edge plus whichever of its two corner arcs it
// owns, for the general per-side path below) -- and paint a dash/dot pattern
// along it, phased continuously off one distance coordinate from the start of
// the sequence.
static void walkAndDashSegments(LVDrawBuf & drawbuf, const CRPerimeterSeg segs[], int segCount, bool wrap,
                                 int w, bool dotted, lUInt32 color)
{
    ...
    // DASHED: collect every dash-on run [distStart, distEnd) (distances along
    // the sequence) and hand them all to strokeDashRunsFT() in one FT_Stroker
    // pass -- see the comment above strokeDashRunsFT for why FT_Stroker
    // replaces the old AABB-quad walk for this case. Run detection itself
    // (the totalSteps loop below) is unchanged and shared with the dotted
    // case: neither wrap (a closed ring's dash phase isn't merged across the
    // totalLen/0 seam either, in this implementation or the old one) nor the
    // dash/gap timing depend on the rendering method.
    std::vector<std::pair<double, double> > dashRuns;

    // DOTTED: point + inward unit vector at distance d along the sequence,
    // wrapped into [0, totalLen) for a closed loop, clamped to it for an open
    // chain (d is never asked for outside that range anyway, since the walk
    // below only steps from 0 to totalLen). This is what lets a dot be
    // sampled correctly even when its run straddles two segments -- the
    // lookup itself crosses the seam. Unused (and unneeded) for the dashed
    // case, which is stroked as a path rather than sampled point-by-point.
    auto pointAt = [&](double d, double &px, double &py, double &iux, double &iuy) {
        ...
        if (wrap) {
            ...
            dd = d < 0 ? 0 : (d > totalLen ? totalLen : d);
        }
        ...
        // is never actually walked -- a distance that lands exactly on one
        // (e.g. d==0 at the start of an open chain beginning with a
        // zero-radius corner arc) resolves to the nearest real edge/arc
        // instead, carrying the phase straight from the end of one straight
        // edge into the start of the next.
    ...
    // Flush one dash-on run [distStart, distEnd) (both are distances along
    // the sequence). segs[] walks the border's *centerline* (see
    // computeRoundRectCenterline / CRRoundRectCenterline), so a sampled point
    // already sits w/2 in from the outer edge. Dotted: one round dab of
    // radius w/2 centered right on the sampled point, via pointAt, so it's
    // correctly placed even when the run's midpoint falls in a different
    // segment than where the run started. Dashed: the run is only recorded
    ...
    auto flushRun = [&](double distStart, double distEnd) {
        ...
        dashRuns.push_back(std::make_pair(distStart, distEnd));
    };
    ...
    if (!dotted) {
        ...
        bool fullLoopRun = wrap && dashRuns.size() == 1 &&
                                       ...
                                       const int rx[4], const int ry[4], int w, bool dotted, lUInt32 color)
{
    ...
    walkAndDashSegments(drawbuf, segs, segCount, true, w, dotted, color);
}
...
void DrawBorder(ldomNode *enode,LVDrawBuf & drawbuf,int x0,int y0,int doc_x,int doc_y,RenderRectAccessor fmt)
{
    ...
    if (hasbottomBorder || hasleftBorder || hasrightBorder || hastopBorder) {
        ...
        // Rounded border paths, tried in order, each drawing the whole ring
        // and returning: a uniform single-color solid ring, a uniform
        // dashed/dotted ring, then the general per-side path (any mix of
        // solid/inset/outset/dashed/dotted/double/groove/ridge). Every
        // border style is covered by one of these once a radius is present,
        // so the legacy square-corner rendering below is only reached when
        // there's no border-radius at all, or no border to draw.
        if (styleHasBorderRadii(style.get())) {
            ...
            if (rx[0]||rx[1]||rx[2]||rx[3]||ry[0]||ry[1]||ry[2]||ry[3]) {
                ...
                // checked above (plus absent), so this branch is unconditional
                ...
                if (side_coverable[0] && side_coverable[1] && side_coverable[2] && side_coverable[3]) {
                    ...
                    // DASHED/DOTTED, any side: walk each dashed/dotted side's own edge
                    // plus whichever of its two corner arcs it owns, via
                    // walkAndDashSegments, the same continuous-perimeter-distance
                    // approach fillRoundedRectDashedRing uses for the uniform case,
                    // instead of phasing by row/column per side (which put a visible
                    // seam right where each edge met its corner arc). When a corner is
                    // shared by two active sides (any styles, not just dashed/dotted
                    // ones), the arc is split at its 45-degree bisector, one half to each
                    // side -- the dashed/dotted side's own chain gets its half via
                    // walkAndDashSegments below, and the non-dashed neighbor's own ring
                    // fill already curves into its own half via the dashed side's true
                    // (miter-only) width -- see fillRoundedRectBorder's w_*/mw_* split.
                    // Only when a dashed/dotted side has *no* neighbor at that corner
                    // (the neighbor's border is absent entirely) does it keep the whole
                    // arc, since nothing else will paint any of it.
                    if (side_is_dashed[0] || side_is_dotted[0] || side_is_dashed[1] || side_is_dotted[1] ||
                        ...
                        bool top_dashed = side_is_dashed[0] || side_is_dotted[0];
                        bool right_dashed = side_is_dashed[1] || side_is_dotted[1];
                        bool bottom_dashed = side_is_dashed[2] || side_is_dotted[2];
                        bool left_dashed = side_is_dashed[3] || side_is_dotted[3];

                        // Each arc's t0..t1 range starts adjacent to its counter-clockwise
                        // neighbor and ends adjacent to its clockwise neighbor (see the
                        // clockwise segment order in fillRoundedRectDashedRing); tmid is the
                        // 45-degree bisector where a mitered corner's seam belongs.
                        const double tTL0 = CRE_PI, tTL1 = 3.0 * CRE_PI / 2.0, tTLm = (tTL0 + tTL1) / 2.0; // TL: left..top
                        const double tTR0 = -CRE_PI / 2.0, tTR1 = 0.0, tTRm = (tTR0 + tTR1) / 2.0;         // TR: top..right
                        const double tBR0 = 0.0, tBR1 = CRE_PI / 2.0, tBRm = (tBR0 + tBR1) / 2.0;          // BR: right..bottom
                        const double tBL0 = CRE_PI / 2.0, tBL1 = CRE_PI, tBLm = (tBL0 + tBL1) / 2.0;       // BL: bottom..left

                        bool splitTL = hasleftBorder && hastopBorder;
                        bool splitTR = hastopBorder && hasrightBorder;
                        bool splitBR = hasrightBorder && hasbottomBorder;
                        bool splitBL = hasbottomBorder && hasleftBorder;

                        bool topOwnsTL = top_dashed && (splitTL || !hasleftBorder);
                        bool topOwnsTR = top_dashed && (splitTR || !hasrightBorder);
                        bool rightOwnsTR = right_dashed && (splitTR || !hastopBorder);
                        bool rightOwnsBR = right_dashed && (splitBR || !hasbottomBorder);
                        bool bottomOwnsBR = bottom_dashed && (splitBR || !hasrightBorder);
                        bool bottomOwnsBL = bottom_dashed && (splitBL || !hasleftBorder);
                        bool leftOwnsBL = left_dashed && (splitBL || !hasbottomBorder);
                        bool leftOwnsTL = left_dashed && (splitTL || !hastopBorder);

                        // Each side's edge, plus whichever corner arc(s) it owns, is walked
                        // along *its own* centerline -- computeRoundRectCenterline as if the
                        // whole ring shared that side's width -- rather than the outer box.
                        // A corner an active side owns alone (no bordered neighbor there)
                        // then centerlines exactly like the uniform-ring case above; a split
                        // corner instead gets two independently-computed centerline arcs
                        // meeting only approximately at the 45-degree seam when the two
                        ...
                        // A side only adds its owned corner arc to its chain when that
                        // corner's centerline radius is actually non-degenerate (see
                        // crCenterlineHasArc) -- a collapsed-to-a-point corner needs no
                        // segment at all, since the edge either side of it already meets
                        // exactly there.
                        if (top_dashed) {
                            CRPerimeterSeg chain[3];
                            int n = 0;
                            if (topOwnsTL && crCenterlineHasArc(gT, 0))
                                chain[n++] = segTL_top;
                            chain[n++] = segTop;
                            if (topOwnsTR && crCenterlineHasArc(gT, 1))
                                chain[n++] = segTR_top;
                            walkAndDashSegments(drawbuf, chain, n, false, tbw, side_is_dotted[0], sideColors[0]);
                        }
                        if (right_dashed) {
                            CRPerimeterSeg chain[3];
                            int n = 0;
                            if (rightOwnsTR && crCenterlineHasArc(gR, 1))
                                chain[n++] = segTR_right;
                            chain[n++] = segRight;
                            if (rightOwnsBR && crCenterlineHasArc(gR, 2))
                                chain[n++] = segBR_right;
                            walkAndDashSegments(drawbuf, chain, n, false, rbw, side_is_dotted[1], sideColors[1]);
                        }
                        if (bottom_dashed) {
                            CRPerimeterSeg chain[3];
                            int n = 0;
                            if (bottomOwnsBR && crCenterlineHasArc(gB, 2))
                                chain[n++] = segBR_bottom;
                            chain[n++] = segBottom;
                            if (bottomOwnsBL && crCenterlineHasArc(gB, 3))
                                chain[n++] = segBL_bottom;
                            walkAndDashSegments(drawbuf, chain, n, false, bbw, side_is_dotted[2], sideColors[2]);
                        }
                        if (left_dashed) {
                            CRPerimeterSeg chain[3];
                            int n = 0;
                            if (leftOwnsBL && crCenterlineHasArc(gL, 3))
                                chain[n++] = segBL_left;
                            chain[n++] = segLeft;
                            if (leftOwnsTL && crCenterlineHasArc(gL, 0))
                                chain[n++] = segTL_left;
                            walkAndDashSegments(drawbuf, chain, n, false, lbw, side_is_dotted[3], sideColors[3]);
                        }
                    }
                    ...
                    // with each corner radius reduced by the same amount. Each line uses
                    // fillRoundedRectBorder's simultaneous, both-axes-at-once corner
                    // inset rather than a per-side "own reach" one: sharing a single
                    // box/radii across the outer/gap/inner sub-bands leaves the *gap*
                    // band's curve at the mercy of two independently-computed,
                    // axis-blind boundaries that don't line up, which either bleeds the
                    // outer line into the gap at the corner or squares off the inner
                    // line's own corner. Treating each line as its own small rounded
                    // rect sidesteps both: it's the same trick that already makes
                    ...
                    if (side_is_double[0] || side_is_double[1] || side_is_double[2] || side_is_double[3]) {
                        ...
                        // (e.g. solid) -- so this ring's corner curve reserves the
                        // correct reach for that neighbor instead of assuming it
                        // contributes nothing just because it isn't double itself.
                        // Only a side that's actually double is drawn.
                        ...
                        side_is_ridge[0] || side_is_ridge[1] || side_is_ridge[2] || side_is_ridge[3])
                    {
                        ...
                        // rationale as DOUBLE just above. vouter_*/vinner_* feed both the
                        // inset that positions the inner ring's box and the widths passed
                        // to each ring call; draw_t/draw_r/draw_b/draw_l gate actual
                        // painting on the side's *real* style.
...
void DrawBackgroundImage(ldomNode *enode,LVDrawBuf & drawbuf,int x0,int y0,int doc_x,int doc_y, int width, int height, bool clip_to_target=true)
{
    ...
    if (!style->background_image.empty()) {
        ...
        if (!img.isNull()) {
            ...
            // does for <img> elements (otherwise, on a screen with gRenderDPI != 96, a
            // background-image with no background-size would be drawn at its raw pixel
            // size instead of matching the device's actual pixel density).
            ...
            if ( draw_bg_color || draw_bg_image ) {
                if ( enode->getNodeId() == el_body ) {
                    ...
                    if ( draw_bg_color ) {
                        ...
                        // as a rounded ring: the single-color solid fast path, the uniform
                        // dashed/dotted fast path, or the general path (every present side
                        // solid/inset/outset/dashed/dotted/double/groove/ridge -- any mix,
                        // colors/widths may differ per side). Every border style is covered
                        // by one of these once a radius is present, so this effectively
                        // always rounds whenever there's a border at all; the explicit
                        // checks are kept so this stays in lockstep with DrawBorder() rather
                        // than assuming that coverage.
                        int rx[4]={0,0,0,0}, ry[4]={0,0,0,0};
                        if ( styleHasBorderRadii(style.get()) ) {
                            bool no_border = !styleHasAnyBorder(style.get());
                            bool rounded_border = false;
                            if ( !no_border ) {
                                int w_top, w_right, w_bottom, w_left; lUInt32 ring_color;
                                rounded_border = styleQualifiesForFastRoundedBorder(enode, style.get(), w_top, w_right, w_bottom, w_left, ring_color);
                                if ( !rounded_border ) {
                                    int ring_width; bool dotted;
                                    rounded_border = styleQualifiesForFastDashedRoundedBorder(enode, style.get(), ring_width, ring_color, dotted);
                                }
                                if ( !rounded_border )
                                    rounded_border = styleHasOnlyRoundableBorders(style.get());
                            }
                            if (no_border || rounded_border)
                                computeBorderRadiiPx(enode, style.get(), fmt.getWidth(), fmt.getHeight(), rx, ry);
                        }
                        if (rx[0]||rx[1]||rx[2]||rx[3]||ry[0]||ry[1]||ry[2]||ry[3])
