...
// Whether every *present* border side is solid, inset, or outset (any mix of
// the three is fine) -- i.e. whether DrawBorder() would render this box's
// border via the general fillRoundedRectBorder() path rather than falling
// back to the legacy square-corner code for a dashed/dotted/double/groove/
// ridge side. Used by the background-fill code (which has no border-drawing
// context of its own) to decide whether its fill must be rounded to match.
static bool styleHasOnlySolidInsetOutsetBorders(const css_style_rec_t * style) {
    for (int i=0; i<4; i++) {
...
void DrawBorder(ldomNode *enode,LVDrawBuf & drawbuf,int x0,int y0,int doc_x,int doc_y,RenderRectAccessor fmt)
{
    ...
    if (hasbottomBorder || hasleftBorder || hasrightBorder || hastopBorder) {
        ...
        // and returning. Any style/mix these don't cover (dashed/dotted
        // sides mixed with other styles or disagreeing on width/color,
        // double, groove, or ridge) falls back to the existing square-corner
        // rendering below, even if the style also specifies a border-radius.
        if (styleHasBorderRadii(style.get())) {
            ...
            if (rx[0]||rx[1]||rx[2]||rx[3]||ry[0]||ry[1]||ry[2]||ry[3]) {
                ...
                // General per-side path for SOLID/INSET/OUTSET: every
                // *present* side must be one of these three styles (any mix
                // of them is fine, with independent colors/widths per side --
                // fillRoundedRectBorder() miters between differing colors, and
                // inset/outset get their light/shade tint per side below). If
                // any present side is dashed/dotted/double/groove/ridge, this
                // box isn't fully covered by rounded rendering yet, so leave
                // it entirely to the legacy square-corner code below rather
                // than draw some sides rounded and silently skip others.
                ...
                if (side_ok[0] && side_ok[1] && side_ok[2] && side_ok[3]) {
                    ...
                    lUInt32 c[4] = {topBordercolor, rightBordercolor, bottomBordercolor, leftBordercolor};
                    if (invert_colors) {
                        for (int i=0; i<4; i++)
                    ...
                    fillRoundedRectBorder(drawbuf, X0, Y0, X1, Y1, rx, ry,
                                              mw[0], hastopBorder, mw[1], hasrightBorder, mw[2], hasbottomBorder, mw[3], hasleftBorder, c);
                    return;
                }
            }
        }
            ...
            if ( draw_bg_color || draw_bg_image ) {
                if ( enode->getNodeId() == el_body ) {
                    ...
                    if ( draw_bg_color ) {
                        ...
                        // dashed/dotted fast path, or the general solid/inset/outset path
                        // (every present side one of those three styles, colors/widths may
                        // differ per side). Any other border (a present side that's
                        // dashed/dotted mixed with other styles or disagreeing on
                        // width/color, double, groove, or ridge) is still drawn square by
                        ...
                        if ( styleHasBorderRadii(style.get()) ) {
                            ...
                            if ( !no_border ) {
                                ...
                                if ( !rounded_border )
                                    rounded_border = styleHasOnlySolidInsetOutsetBorders(style.get());
                            }
